 ...
 */
public class OutputFieldLinkDefinition<T extends Type<T, O>, O extends Obj<T, O>> extends FieldLinkDefinition {

    private static final Logger log = LoggerFactory.getLogger(OutputFieldLinkDefinition.class);
    ...
    public void link(Function<O, FieldInput> pathProvider) {
        this.pathProvider = pathProvider;
    }

    public Function<O, FieldInput> getPathProvider() {
        return pathProvider;
    }

    public void instantiate(Field f) {
        FieldInput output = pathProvider.apply((O) f.getObject());
        instantiateAndLink(f, output);
    }
