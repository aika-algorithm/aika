...
Type::Type(std::shared_ptr<TypeRegistry> registry, const std::string& name)
    ...
    id = registry->registerType(shared_from_this());
}
...
void Type::initFlattenedType() {
    ...
    flattenedTypeInputSide = createInputFlattenedType(shared_from_this(), fieldDefs);
    flattenedTypeOutputSide = createOutputFlattenedType(shared_from_this(), fieldDefs, flattenedTypeInputSide);
}
...
void Type::collectTypesRecursiveStep(std::set<std::shared_ptr<Type>>& sortedTypes) {
    ...
    sortedTypes.insert(shared_from_this());
}
